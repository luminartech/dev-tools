...
Sync VSCode settings and extensions from `devcontainer.json` to `.vscode` folder. `devcontainer.json` will be now your source of truth. This hook runs in `post-checkout` and `post-merge` stages, so consider installing it with `pre-commit -install -t` or defining your `default_install_hook_types` in the config.
